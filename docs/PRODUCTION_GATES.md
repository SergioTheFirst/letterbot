...
- `config-ready` and `validate-config` must fail fast with explicit field-level output.
- New-install flow must not require `config.yaml`.

## G3 — LAN UI safety
- LAN bind rules and CIDR restrictions enforced through `settings.ini`.
