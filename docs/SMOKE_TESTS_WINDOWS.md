...
- Action: Run `letterbot.bat` (source) or `run.bat` (extracted dist ZIP).
- Expected: Launcher creates missing INI files, runs `config-ready`, and opens `accounts.ini` when required fields are missing.
