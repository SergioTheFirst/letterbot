...
Система обязана уведомить пользователя о факте и сути нового письма как можно раньше, даже если глубокий анализ ещё не завершён.

Быстрое уведомление должно быть:
- Корректным — без галлюцинаций, только извлечённые факты
- Объяснимым — rule-based логика с явными reason codes
- Безопасным — no binary leaks, HTML-escaped, graceful fallback

Глубокий анализ (контекст, паттерны, рекомендации) может приходить отдельным последующим сообщением.

Важно: Fast First не отменяет качество. Если для wow-результата нужно 2–3 минуты вместо 5 секунд — это допустимо. Принцип означает “не задерживай пользователя, если можешь не задерживать”.
...
Запрещены параллельные обращения в GigaChat (Sber):

риск блокировки доступа.

Разрешены только последовательные запросы.
