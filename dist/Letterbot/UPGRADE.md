...
3. Copy only user config files from the old folder:
   - `mailbot_v26/config/settings.ini`
   - `mailbot_v26/config/accounts.ini`
4. Open the extracted `Letterbot` folder, run `run.bat`, and confirm startup checks pass.
...
2. Recreate them from `settings.ini.example` / `accounts.ini.example`.
