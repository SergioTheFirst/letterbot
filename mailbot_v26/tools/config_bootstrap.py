...
[telegram_ui]
; Show engineering DecisionTrace in Telegram action row (false = hidden in normal UX).
...
text = LetterBot.ru остаётся бесплатным. Спасибо за поддержку развития.
; The support URL is fixed in source (mailbot_v26/config_loader.py)
; and always points to the official LetterBot.ru project page.
; This setting is read-only and cannot be changed here.
; url = https://boosty.to/personalbot/donate?qr=true  [hardcoded]
...
message = Поддержать LetterBot.ru → https://letterbot.ru
