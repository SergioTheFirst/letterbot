...
def test_events_default_window(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with app.test_client() as client:
        ...
        page_response = client.get(
            "/events", query_string={"account_emails": "primary@example.com"}
        )
        assert page_response.status_code == 200
        page_text = page_response.get_data(as_text=True)
...
def test_events_narrative_ordering_and_determinism(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    app = _build_app(tmp_path)
    monkeypatch.setattr(
    ...
    with app.test_client() as client:
        ...
        assert (
            first_text.index("Email 3")
            < first_text.index("Email 2")
            < first_text.index("Email 1")
        )


def test_events_narrative_pagination_stable(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    app = _build_app(tmp_path)
    monkeypatch.setattr(
...
def test_events_page_renders_grouped_cards(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    app = _build_app(tmp_path)
    monkeypatch.setattr(
    ...
    with app.test_client() as client:
        ...
        assert (
            "Grouped timeline keeps related events together for easier review." in body
        )
        assert "Entries" in body
        assert "Window" in body


def test_events_empty_state_safe(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    app = _build_app(tmp_path)
    monkeypatch.setattr(
    ...
    with app.test_client() as client:
        login_with_csrf(client, "pw")
        page = client.get(
            "/events", query_string={"account_emails": "missing@example.com"}
        )
        body = page.get_data(as_text=True)
        assert page.status_code == 200
