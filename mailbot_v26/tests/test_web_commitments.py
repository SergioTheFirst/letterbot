    ...
    *,
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            ...
            VALUES (?, ?, ?, ?)
            """,
            (email_id, account_email, from_email, received_at.isoformat()),
        )
    ...
    *,
    ...
    with sqlite3.connect(db_path) as conn:
        conn.execute(
            ...
            ) VALUES (?, ?, 'heuristic', 'Send report', ?, ?, 0.9, ?)
            """,
            (
                commitment_id,
                email_id,
...
def test_commitments_auth_required(tmp_path: Path) -> None:
    ...
    _insert_commitment(
        ...
        commitment_id=1,
        email_id=1,
        deadline_iso=now.date().isoformat(),
        status="pending",
        created_at=now,
    )
...
def test_commitments_deterministic_ordering_and_pagination(tmp_path: Path) -> None:
    db_path, app = _build_app(tmp_path)
    now = datetime.now(timezone.utc)
    for email_id in range(1, 56):
        _insert_email(
            db_path,
            email_id=email_id,
            account_email="acct@example.com",
        ...
        _insert_commitment(
            db_path,
            commitment_id=email_id,
            email_id=email_id,
            deadline_iso="2026-01-10",
            status="pending",
            created_at=now,
        )
...
def test_email_forensics_evidence_card(tmp_path: Path) -> None:
    db_path, app = _build_app(tmp_path)
    now = datetime.now(timezone.utc)
    _insert_email(
        db_path,
    ...
    with app.test_client() as client:
        ...
        assert "processing_span" in body
